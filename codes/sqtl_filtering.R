...
for (i in 1:22){
  ...
  sample_expressions <- as.data.frame(read.table(file = paste("/home/ryan/test_runs/juncfiles/Gene_expression_chr",i,sep=""), sep = '\t', header =T))
  reference_genotype <- as.data.frame(read.table(file = paste("/home/ryan/DATA/SNP_genotype_chr",i,".txt",sep=""), header=T))
